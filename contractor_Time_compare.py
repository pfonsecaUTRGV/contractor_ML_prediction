...
def evaluate_model(model, X_train, X_test, y_train, y_test):
    ...
    return {
        ...
        'Accuracy': tolerance_accuracy(y_test, preds)
    }

# =======================================
# 2️⃣ Prepare datasets (with and without time features)
# =======================================
X_with_time = X  # Already has categorical + WBS + time features
X_no_time = np.hstack([cat_features, wbs_reduced])  # Remove time info

split_index = int(len(df) * 0.8)
X_train_t, X_test_t = X_with_time[:split_index], X_with_time[split_index:]
y_train_t, y_test_t = y[:split_index], y[split_index:]

X_train_nt, X_test_nt = X_no_time[:split_index], X_no_time[split_index:]
y_train_nt, y_test_nt = y[:split_index], y[split_index:]
...
X_train_nt = scaler_nt.fit_transform(X_train_nt)
X_test_nt = scaler_nt.transform(X_test_nt)
...
models = {
    ...
    "XGBoost": XGBRegressor(random_state=42, n_estimators=200, max_depth=6, learning_rate=0.1),
...
# =======================================
# 4️⃣ Evaluate all models with/without time
# =======================================
results = []

for name, model in models.items():
    print(f"\n Training {name} WITHOUT time features...")
    res_no_time = evaluate_model(model, X_train_nt, X_test_nt, y_train_nt, y_test_nt)
    res_no_time['Model'] = name
    res_no_time['Setting'] = 'Without Time'

    print(f" Training {name} WITH time features...")
    res_time = evaluate_model(model, X_train_t, X_test_t, y_train_t, y_test_t)
    res_time['Model'] = name
    res_time['Setting'] = 'With Time'

    results.append(res_no_time)
    results.append(res_time)

results_df = pd.DataFrame(results)
results_df = results_df[['Model', 'Setting', 'MAE', 'RMSE', 'R2', 'Accuracy']]

# =======================================
# 5️⃣ Print comparison table
# =======================================
...
for i, metric in enumerate(metrics):
    ax = axes[i // 2, i % 2]
    sns.barplot(data=results_df, x='Model', y=metric, hue='Setting', palette='viridis', ax=ax)
    ax.set_title(f"{metric} Comparison (With vs Without Time)")
    ax.set_xlabel('')
